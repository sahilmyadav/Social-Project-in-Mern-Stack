...
if (cluster.isPrimary) {
  ...
  import('./index.js')
    .then(() => {})
    .catch((error) => {
      logger.error(`Worker #${workerId} (PID: ${process.pid}) failed to start:`, {
        error: error.message,
      });
      process.exit(1);
    });
  ...
  process.on('uncaughtException', (error) => {
    logger.error(`Worker #${workerId} uncaught exception:`, { error: error.message });
    process.exit(1);
  });

  process.on('unhandledRejection', (reason, promise) => {
    logger.error(`Worker #${workerId} unhandled rejection:`, { reason: String(reason) });
    process.exit(1);
  });
}
