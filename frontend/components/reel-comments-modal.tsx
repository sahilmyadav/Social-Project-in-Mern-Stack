...
export default function ReelCommentsModal({
  ...
  const handleSubmitReply = async (e: React.FormEvent) => {
    ...
    try {
      const response = await commentService.replyToComment(replyingTo.commentId, { text: replyText.trim() })
      if (response.success) {
        setReplyText("")
  ...
  const handleLikeComment = async (commentId: string) => {
    try {
      ...
      setComments(prev => prev.map(c =>
        c._id === commentId
          ? { ...c, isLiked: !c.isLiked, likes_count: c.isLiked ? Math.max(0, (c.likes_count || 1) - 1) : (c.likes_count || 0) + 1 }
          : c
      ))
      ...
      setRepliesData(prev => {
        ...
        for (const [parentId, replies] of newMap.entries()) {
          const updatedReplies = replies.map((r: any) =>
            r._id === commentId
              ? { ...r, isLiked: !r.isLiked, likes_count: r.isLiked ? Math.max(0, (r.likes_count || 1) - 1) : (r.likes_count || 0) + 1 }
              : r
          )
                          ...
                          <div key={reply._id} className="flex gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-3 mt-1 px-2">
                                ...
                                >
                                  <Heart size={10} className={reply.isLiked ? 'fill-primary text-primary' : ''} />
        ...
        <div className="px-4 py-3 border-t border-border bg-background">
          ...
          <form onSubmit={replyingTo ? handleSubmitReply : handleSubmitComment} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-muted rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary border border-border"
              disabled={submitting || submittingReply}
            />
            <button
              type="submit"
              disabled={replyingTo ? (!replyText.trim() || submittingReply) : (!newComment.trim() || submitting)}
