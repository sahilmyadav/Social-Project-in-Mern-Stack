...
export function EnhancedImageEditor({
  isOpen,
  onClose,
  imageFile,
  ...
  const getCanvasDimensions = useCallback(() => {
    if (type === 'profile') {
      ...
      // Cover: 2.5:1 aspect ratio
      return { width: 400, height: 160 };
    }
  }, [type]);

  // Load image when file or src changes
  useEffect(() => {
  ...
  }, [imageFile, propImageSrc, isOpen, type]);
  ...
  // Face detection using canvas (simple heuristic-based approach)
  // For production, you'd use a proper face detection library like face-api.js
  const detectFaceAndCenter = async (img: HTMLImageElement) => {
    ...
    try {
      ...
      // If we found a reasonable face region (at least 5% of image)
      ...
      if (skinPixels > totalPixels * 0.05 && maxX > minX && maxY > minY) {
        ...
        // Center the face in the crop area
        ...
        // Calculate optimal zoom to fit face with some padding
        const faceSize = Math.max(faceRect.width, faceRect.height);
        const targetSize = Math.min(canvasDims.width, canvasDims.height) * 0.6;
        const optimalZoom = Math.min(Math.max(targetSize / faceSize, 1), 2);
        
        // Set position to center face
        const offsetX = (imgCenterX - faceCenterX) * optimalZoom * 0.5;
        const offsetY = (imgCenterY - faceCenterY) * optimalZoom * 0.5;
  ...
  // Draw canvas with all transforms
  const drawCanvas = useCallback(() => {
    ...
    if (imgAspect > canvasAspect) {
      drawHeight = canvas.height * zoom * 1.2; // Add some extra coverage
  ...
  }, [originalImage, imageLoaded, rotation, fineRotation, flipH, flipV, zoom, position, type, showGrid, getCanvasDimensions]);
  ...
  const handleAutoFocus = async () => {
    if (originalImage) {
      await detectFaceAndCenter(originalImage);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      // Create a clean canvas without grid for saving
      const saveCanvas = document.createElement('canvas');
      const saveCtx = saveCanvas.getContext('2d');
      if (!saveCtx || !originalImage) return;

      const dims = getCanvasDimensions();
      saveCanvas.width = dims.width;
      saveCanvas.height = dims.height;
      ...
      // Apply circular mask for profile
      if (type === 'profile') {
        saveCtx.save();
        saveCtx.globalCompositeOperation = 'destination-in';
        saveCtx.beginPath();
        saveCtx.arc(saveCanvas.width / 2, saveCanvas.height / 2, saveCanvas.width / 2, 0, Math.PI * 2);
        saveCtx.fill();
        saveCtx.restore();
      }
      ...
      saveCanvas.toBlob(
        (blob) => {
          if (blob) {
            const previewUrl = saveCanvas.toDataURL('image/jpeg', 0.95);
      ...
      <DialogContent className="max-w-[95vw] md:max-w-2xl p-0 gap-0 overflow-hidden bg-zinc-950 border-zinc-800">
        ...
        <div className="flex flex-col">
          ...
          <div
            ...
            style={{ minHeight: type === 'profile' ? '340px' : '200px' }}
          >
            {/* Circular/Rectangular Frame */}
            <div
              className={cn(
                'relative overflow-hidden shadow-2xl',
                type === 'profile' ? 'rounded-full ring-4 ring-zinc-700' : 'rounded-xl ring-2 ring-zinc-700'
              )}
              style={{
                width: type === 'profile' ? '300px' : '100%',
                maxWidth: type === 'profile' ? '300px' : '400px',
              }}
            >
              <canvas
                ref={canvasRef}
          ...
          <div className="flex border-b border-zinc-800">
            {[
              { id: 'crop', label: 'Crop & Position', icon: Move },
