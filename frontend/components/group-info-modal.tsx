...
export default function GroupInfoModal({
  ...
  const isAdmin =
    groupDetails?.members?.some((member: any) => {
      ...
      return userId === currentUserId && (member.role === 'admin' || member.role === 'owner');
    }) || groupDetails?.createdBy?._id === currentUserId;

  const isOwner = groupDetails?.createdBy?._id === currentUserId;
              ...
              <div className="flex flex-col items-center gap-4">
                <div className="relative">
                  <div className="w-24 h-24 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center overflow-hidden">
                    {groupDetails?.avatar ? (
                      <img
                        src={getMediaUrl(groupDetails.avatar)}
                        alt="Group avatar"
              ...
              <div>
                ...
                <div className="space-y-2">
                  {members.map((member, index) => {
                    ...
                    const rawAvatar =
                      memberUser.profileImage || memberUser.profilePicture || memberUser.avatar;
                    const memberAvatar = rawAvatar
                    ...
                    return (
                      ...
                      >
                        <img
                          src={memberAvatar}
                          alt={memberUser.firstName || 'Member'}
                          className="w-12 h-12 rounded-full object-cover"
                        />
                        <div className="flex-1">
                          <p className="font-semibold text-gray-900 dark:text-white">
                            {memberUser.firstName || 'Unknown'} {memberUser.lastName || ''}
                            {isCurrentUser && (
                        ...
                        {isAdmin && !isCurrentUser && !memberIsOwner && (
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              {!memberIsAdmin && (
                                <DropdownMenuItem onClick={() => handleMakeAdmin(memberId)}>
