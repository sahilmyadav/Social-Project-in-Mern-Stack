...
export default function VoiceCallModal({
  ...
  useEffect(() => {
    console.log('🎤 VoiceCallModal isOpen:', isOpen, 'isIncoming:', isIncomingCall)
            ...
            const handleAnswer = async (answerData: any) => {
              try {
                ...
                if (!peerConnection || peerConnection.connectionState === 'closed' || peerConnection.connectionState === 'failed') {
                  console.log('⚠️ Ignoring answer - connection is closed/failed:', peerConnection?.connectionState)
                  return
                }
                
                // Check if we're in the correct state to receive an answer
                if (peerConnection.signalingState !== 'have-local-offer') {
                  console.log('⚠️ Ignoring answer - signaling state is:', peerConnection.signalingState)
                  return
                }
      ...
      const processOffer = async (offerData: any, peerConnection: RTCPeerConnection) => {
        try {
          ...
          if (peerConnection.signalingState !== 'stable') {
            console.log('⚠️ Cannot process offer - signaling state is:', peerConnection.signalingState)
            return
          }
  ...
  const toggleSpeaker = () => {
    console.log('🔈 Toggling speaker:', !isSpeakerOn)
  ...
  return (
    <div className='fixed inset-0 z-[9999] bg-black flex items-center justify-center'>
      ...
      <button
        onClick={onClose}
        className='absolute top-6 right-6 p-3 rounded-full bg-white/10 hover:bg-white/20 transition z-50'
        title='Close call'
      >
      ...
      <div className='relative flex flex-col items-center justify-center w-full h-full max-w-md'>
        ...
        <div className='flex flex-col items-center mb-12'>
          ...
          <div className='relative mb-8'>
            <div className='w-32 h-32 rounded-full bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 flex items-center justify-center text-7xl shadow-2xl border-4 border-white/10'>
            ...
            {callStatus === 'ringing' && isIncomingCall && (
              <>
                ...
                <button
                  ...
                  className='w-16 h-16 rounded-full bg-green-500 hover:bg-green-600 active:scale-95 flex items-center justify-center transition transform shadow-xl hover:shadow-green-500/50'
                  title='Accept call'
                >
